...
export async function POST(req: Request) {
  try {
    ...
    const { clientId, startDate, endDate, workoutContent, additionalInfo, documentName } = await req.json()
    ...
    // Use custom document name or create default: "ClientName Workout StartDate-EndDate"
    const finalDocumentName = documentName || (() => {
      ...
      return `${client.name} Workout ${startDateFormatted} to ${endDateFormatted}`
    })()
